...
class discohash(plugins.Plugin):
    ...
    __license__ = 'GPL3'
    __description__ = '''
                    This Plugin extracts hashes from pcaps (hashcat mode 22000) using hcxpcapngtool,
    ...
    def process_pcaps(self, handshake_dir):
        ...
        for num, handshake in enumerate(handshakes_list):
            ...
            if ((num + 1) % 10 == 0) or (num + 1 == len(handshakes_list)):
                logging.debug('[*] DiscoHash Batch job: {}/{} done ({} fails)'.format(num + 1, len(handshakes_list),
                                                                                      len(lonely_pcaps)))
        if successful_jobs:
            logging.debug('[*] Batch job: {} new handshake files created'.format(len(successful_jobs)))
        if lonely_pcaps:
            logging.debug('[*] Batch job: {} networks without enough packets to create a hash'.format(
                len(lonely_pcaps)))
    ...
    def get_coord(self, fullpathNoExt):
        ...
        try:
            ...
            lat = "NULL"
            lon = "NULL"
            loc_url = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
    ...
    def post_hash(self, fullpathNoExt):
        try:
            ...
            logging.warning('[!] An error occured while analysing the hash: {}'.format(e))
        try:
            data = {
                'embeds': [
                    {
                        ...
                        'footer': {
                            'text': 'Pwnagotchi v1.5.5 - Rishis Plugin for pwnagotchi \
                        \nAuthors PwnMail: f033aa5cd581f67ac5f88838de002fc240aadc74ee2025b0135e5fff4e4b5a4a'.format(
                                self.__version__)
                        }
                    }
                ]
            ...
            logging.warning('[!] An error occured with the plugin!{}'.format(e))